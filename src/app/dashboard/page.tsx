      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '40px 20px' }}>
        ...
        {!isPaid && (
          <div style={{ background: 'linear-gradient(135deg,var(--surface),rgba(240,192,96,0.04))', border: '1px solid rgba(240,192,96,0.2)', borderRadius: 12, padding: '16px 20px', marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
            <div>
              <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--gold)', marginBottom: 2 }}>ðŸ”’ Free plan active</div>
            ...
            {plans.map((plan: any) => (
              <Link key={plan.id} href={`/plan/${plan.id}`} style={{ display: 'flex', alignItems: 'center', gap: 16, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '16px 20px', textDecoration: 'none', transition: 'border-color 0.15s' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--body)', marginBottom: 3 }}>{plan.goal} â€” {plan.country}</div>
                  <div style={{ fontSize: 13, color: 'var(--muted)' }}>{plan.timeframe} Â· Risk {plan.risk}/10 Â· {plan.currency}{Number(plan.income).toLocaleString()}/mo</div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                  ...
                  {plan.is_premium && <span style={{ fontSize: 11, color: 'var(--gold)', background: 'rgba(240,192,96,0.1)', padding: '3px 8px', borderRadius: 100 }}>â˜… Premium</span>}
